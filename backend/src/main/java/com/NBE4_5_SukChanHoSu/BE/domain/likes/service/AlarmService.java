...
public class AlarmService {
            ...
            try {
                ...
                if (!records.isEmpty()) {
                    for (MapRecord<String, Object, Object> record : records) {
                        ...
                        Long fromUserId = Long.valueOf((String) value.get("fromUserId"));
                        Long toUserId = Long.valueOf((String) value.get("toUserId"));
                        String message = (String) value.get("message"); // 메시지
                        String time = (String) value.get("time"); // 시간
    ...
    public void startMatchStreamListener() {
        while (true) {
            try {
                StreamReadOptions options = StreamReadOptions.empty().count(1);
                StreamOffset<String> offset = StreamOffset.create(MATCH_STREAM, ReadOffset.from(lastMatchId));
                List<MapRecord<String, Object, Object>> records = redisTemplate.opsForStream().read(options, offset);

                if (!records.isEmpty()) {
                    for (MapRecord<String, Object, Object> record : records) {
                        Map<Object, Object> value = record.getValue();
                        Long maleUserId = Long.valueOf((String) value.get("maleUserId"));
                        Long femaleUserId = Long.valueOf((String) value.get("femaleUserId"));
                        String messageMale = (String) value.get("messageMale"); // 남자에게 보낼 메시지
                        String messageFemale = (String) value.get("messageFemale"); // 여자에게 보낼 메시지
                        String time = (String) value.get("time"); // 시간
